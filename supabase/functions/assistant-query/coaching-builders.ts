...
2. НЕ выполняй собственных вычислений: не складывай, не вычитай, не выводи проценты и средние — все нужные числа уже посчитаны в данных. Приводи их как есть.
3. НЕ смешивай метрики: «доход/расход за период» и «средний ежемесячный доход/расход» — это РАЗНЫЕ числа, их нельзя сравнивать друг с другом или вычитать одно из другого.
4. Вывод «расходы превышают доход» можно делать ТОЛЬКО из строки «Сбережения за период»: минус — тратит больше, плюс — сберегает. Не выводи это сам из других чисел.
5. Если в данных "Долги: нет" — НЕ упоминай долги и кредиты
6. Если данных нет — НЕ придумывай
7. Отвечай на том же языке, на котором задан вопрос
8. Используй **жирный** для ключевых цифр
9. ОБЯЗАТЕЛЬНО: когда называешь сумму, всегда указывай период дат, к которому она относится (строка "Период анализа" с датами «с … по …»). Пользователь должен понимать, к какому промежутку времени относится каждая цифра.
...
function buildUserContext(profile: UserFinancialProfile, transactions: TxRow[]): string {
  ...
  // Exact date coverage — «за 6 мес.» alone let the LLM invent ranges.
  const dates = transactions.map((t) => t.date.slice(0, 10)).sort();
  const fmtRu = (iso: string) => iso.split('-').reverse().join('.');
  const rangeLabel = dates.length > 0
    ? `с ${fmtRu(dates[0])} по ${fmtRu(dates[dates.length - 1])} (~${monthsWithData} мес.)`
    : 'нет операций';

  // Build human-readable context (NOT JSON) to prevent LLM from doing math
  const lines = [
    `Период анализа: ${rangeLabel}`,
    `Общий доход за период: ${formatMoney(totalIncome)}`,
    `Общие расходы за период: ${formatMoney(totalSpend)}`,
    `Сбережения за период (доход − расход, уже посчитано): ${savings >= 0 ? '+' : ''}${formatMoney(savings)}${savings >= 0 ? ' — пользователь тратит МЕНЬШЕ, чем зарабатывает' : ' — пользователь тратит БОЛЬШЕ, чем зарабатывает'}`,
    `Финансовый этап: ${stageLabels[profile.stage] ?? profile.stage}`,
    `Средний ежемесячный доход (медиана стабильных поступлений, НЕ равен «общий доход ÷ месяцы»): ${formatMoney(profile.monthly_income)}`,
