...
### Тег-триггер: три правила, выученные 2026-09-15 (четыре холостых прогона)

1. **`[skip ci]` в сообщении коммита глушит и tag-build.** Тег на такой
   коммит не запускает `ios-submit-review` вообще — ни check-run, ни письма.
   Тегать только коммит без `[skip ci]`.
2. **Удалённый и пересозданный тег с тем же именем не запускает билд.**
   Для перезапуска — **новое имя** с суффиксом: `release/v1.5.6-r2`.
   Скрипт отбрасывает `-…` при разборе версии, а `PREV_TAG` выбирается по
   базовой версии, так что диапазон «Что нового» не схлопывается.
3. **Провалившийся submit оставляет в ASC пустую версию** (шаг 2 создаёт
   её до поиска билда). Дальше авто-бампер видит коллизию и следующая
   сборка уезжает на патч выше (1.5.5 → 1.5.6). Шаг 2b теперь переименовывает
   такую версию под новый тег вместо создания новой (ASC держит одну «в
   подготовке»).

Рабочая последовательность: push → письмо Codemagic с версией → тег
`release/v<та версия>` на коммит без `[skip ci]` (можно пустой коммит в
боковой ветке — `ios-release` слушает только `main`) → письмо/`gh` о
результате → бамп `project.yml` с `[skip ci]`.
